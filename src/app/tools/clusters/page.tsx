...
export default function ClustersPage() {
  ...
  const submitCluster = async () => {
    if (!textInput.trim()) {
      alert('Please add some text or attach files to analyze');
      return;
    }
    ...
    try {
      ...
      alert(`Clustering analysis failed. Please check your connection and try again.`);
    } finally {
      setLoading(false);
      clearInterval(progressInterval);
      setProgressStep(0);
    ...
    <div className="app-container">
      ...
      <div className="mobile-header">
        <button className="mobile-menu-btn" onClick={toggleSidebar}>
          <div className="sidebar-icon" style={{ transform: sidebarOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}></div>
        </button>
            ...
            <div className="progress-container" style={{ display: 'block' }}>
              <div className="progress-dots">
                {[0, 1, 2, 3].map((index) => (
                  <span 
                    key={index} 
                    className={`progress-dot ${index <= progressStep ? 'active' : ''}`}
                  ></span>
                ))}
              </div>
              <div className="progress-text">
                {['Analyzing patterns...', 'Extracting insights...', 'Building clusters...', 'Generating visualizations...'][progressStep]}
              </div>
            </div>
          )}
        </div>
